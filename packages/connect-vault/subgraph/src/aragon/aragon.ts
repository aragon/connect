import { BigInt, Address, DataSourceTemplate } from '@graphprotocol/graph-ts'
...
export function processApp(appAddress: Address, appId: string, timestamp: BigInt): void {
  if (!_isRegistered(appAddress, 'app')) {
    ...
    if (templateType) {
      ...
      hooks.onAppTemplateCreated(appAddress, appId, timestamp)
    }
