...
export default createAppConnector<Vault, Config>(
  ({ app, config, connector, network, orgConnector, verbose }) => {
    ...
    const connectorTheGraph = new VaultConnectorTheGraph({
      pollInterval,
      subgraphUrl,
      verbose,
    })

    return new Vault(connectorTheGraph, app.address)
  }
)
